...
async function grabTextContent(){ 
  var sel = {text: getSelectionText().trim(), auto: false};
  ...
  if(sel.text == false && isReuters){
    var sel = reutersAutoText();
  }
  if(sel.text == false && isNYT){
    var nyt = await nytAutoText();
    var sel = {text: nyt, auto: true};
  }
  if(sel.text == false && isWP){
    var sel = {text: wpAutoText(), auto: true};
  }
  if(sel.text == false && isRolling){
    var sel = {text: rollstoneAutoText(), auto: true};
  }
  return sel;
}
...
async function playSelection() {
  ...
  var selText = selectedWinText.text ? selectedWinText.text : textDefault;
  ...
  if(selectedWinText.auto) play.click();
}
